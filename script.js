 ...
 * OWNER: ALI MAHFUD
 * VERSION: 5.4 (FINAL STABLE - FULL ESSENCE & INTERACTION FIX)
 */
...
const mindprintDescriptions = {
    ...
    2: { 
        ...
        communication: "Ekspresif, menggunakan bahasa tubuh yang kuat, dan sangat antusias saat berbicara tentang hasil nyata.", 
...
// 1. FUNGSI UNTUK MENGISI DROPDOWN TANGGAL
...
// 3. LOGIKA SCANNER DENGAN PROTEKSI IPAD
const scanner = document.getElementById('fingerprint-scanner');
const scanText = document.getElementById('scan-text');

function startScanning(e) {
    if(e) e.preventDefault(); // SANGAT PENTING: Mencegah menu konteks di iPad
    if(isScanning) return;
    isScanning = true;
    scanner.classList.add('scanning'); // Memicu animasi laser & warna terang di CSS
...
function cancelScanning(e) {
    ...
    isScanning = false;
    scanner.classList.remove('scanning'); // Kembali ke warna samar di CSS
    scanText.textContent = "GAGAL! Jari terlepas. Tempelkan kembali.";
}

if(scanner) {
    // Menangani Touch (Mobile/iPad)
...
document.getElementById('download-btn').addEventListener('click', () => {
    ...
    html2pdf().set(opt).from(el).save().then(() => { document.getElementById('certificate-template').style.display = 'none'; });
});

document.getElementById('restart-button').addEventListener('click', () => location.reload());

// JALANKAN DROPDOWN TANGGAL SAAT HALAMAN DIMUAT
